...
# select * from users;
 ...
 alter table users
        add birthday date default '1900-01-01'                -- добавлен default
		    check (birthday >= '1900-01-01' and birthday < sysdate()),
        add income float;
insert users(first_name, last_name, birthday, user_city, users.income)
	values  ('Иванов', 'Петр', '1980-05-06',1, 1200),
...
# ________________________ДОМАШНЯЯ РАБОТА №8 ___________________________
-- Максимальное и минимальное население в городах
...
-- Выводит уровень населения городов с использованием CASE
SELECT citys.city_name,
       CASE
           WHEN citys.city_population < 100000 THEN 'Население меньше 100 тыс.'
           WHEN citys.city_population >= 100000 AND citys.city_population <= 500000  THEN 'Население от 100 тыс. до 500 тыс.'
           ELSE 'Население больше 500 тыс.'
...
SELECT users.first_name 'ФИО',
    CASE
        WHEN user_city IS NOT NULL THEN city_name
        ELSE 'Город не указан'
        END AS 'Город'
    FROM users
    LEFT JOIN citys
        on users.user_city = citys.city_id;

# ________________________ДОМАШНЯЯ РАБОТА №9 ___________________________
-- Вывести ФИО пользователей проживающих в городах с населением менее 100000
SELECT users.first_name 'ФИО'
    FROM users
